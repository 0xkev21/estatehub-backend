...
if ($method === 'GET') {
    ...
    // Handle Edit (Using POST with _method because PHP doesn't natively parse Multipart PUT)
    $id = $_POST['announcementId'];
    $title = $_POST['title'];
    $desc = $_POST['description'];
    $content = $_POST['announcement'];

    // Optional: Update image if new one provided
    if (isset($_FILES['announcementImage'])) {
        // ... (Move file logic same as previous POST) ...
